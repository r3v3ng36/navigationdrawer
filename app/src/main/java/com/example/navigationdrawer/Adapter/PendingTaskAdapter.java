...
public class PendingTaskAdapter extends RecyclerView.Adapter<PendingTaskAdapter.MyViewHolder> {
        Context context;
        private List<PendingTaskModel> pendingTaskModels;
    public PendingTaskAdapter(Context context, List<PendingTaskModel> pendingTaskModels) {
        this.context = context;
        this.pendingTaskModels = pendingTaskModels;
    }

    @Override
    public PendingTaskAdapter.MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View item = LayoutInflater.from(context)
                .inflate(R.layout.completed_task_row, null,false);
        return new PendingTaskAdapter.MyViewHolder(item);
    }

    @Override
    public void onBindViewHolder(@NonNull PendingTaskAdapter.MyViewHolder myViewHolder, final int i) {
        myViewHolder.tv_employees.setText(pendingTaskModels.get(i).getEmployee());
        myViewHolder.tv_task_detail.setText(pendingTaskModels.get(i).getTask_detail());
        myViewHolder.tv_recording.setText(pendingTaskModels.get(i).getRecording());
        myViewHolder.tv_assign_date.setText(pendingTaskModels.get(i).getAssign_date());
        myViewHolder.tv_completion_date.setText(pendingTaskModels.get(i).getCompletion_date());
        myViewHolder.tv_status.setText(pendingTaskModels.get(i).getStatus());
        ...
        myViewHolder.tv_recording.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("url", "https://step.focusspoint.com/"+pendingTaskModels.get(i).getRecording());
